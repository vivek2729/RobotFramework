...
Globe_Samart_Automation
    ...
    Sleep    20s
    Comment    DatabaseLibrary.Connect To Database    pymysql    nbcu_pilot    vivekkumar.pandey    MMaXm83dZD18A4AA    dev-scraper.csoaie2kvgsc.us-east-1.rds.amazonaws.com
    ...    3306
...
Topics_loop
    ...
    \    Sleep    15s
    \    Comment    DatabaseLibrary.Execute Sql String    INSERT INTO nbcu_pilot.GlobeSmart (`Culture`) VALUES ('${Culture}')
    \    Comment    ${list}=    DatabaseLibrary.Query    select max(id) from nbcu_pilot.GlobeSmart;
    \    Comment    Convert To List    ${list}
    \    Comment    ${list}=    Evaluate    [x[0] for x in ${list}]
    \    Comment    ${Count3}=    Get From List    ${list}    0
    \    Comment    DatabaseLibrary.Execute Sql String    update nbcu_pilot.GlobeSmart set Topic= '${Topic}' where id=${Count3}
    \    Comment    DatabaseLibrary.Execute Sql String    update nbcu_pilot.GlobeSmart set Section= '${Section}' where id=${Count3}
    \    BuiltIn.Run Keyword And Ignore Error    Main_Extraction    ${Culture}    ${Section}    ${Topic}
    \    ${count}    BuiltIn.Evaluate    ${count}+1

Main_Extraction
    [Arguments]    ${Culture}    ${Section}    ${Topic}
    @{List}=    SeleniumLibrary.Get WebElements    //div[@class='concept-grouping util-clearfix']/img
    : FOR    ${j}    IN    @{List}
    \    ${Image_Name}    Get Text    //article/div[@class='concept-grouping util-clearfix'][${count}]/img/following-sibling::p
    \    SeleniumLibrary.Capture Element Screenshot    ${j}    C:/Users/Administrator/Desktop/GlobeSmart_Images/${Culture}_${Section}_${Topic}/${Image_Name}.png
    ...
    \    ...    @{Group_Title_List}
    \    Comment    DatabaseLibrary.Execute Sql String    update nbcu_pilot.GlobeSmart set Details= '${New_gr_Title}' where id=${Count3}
