...
Extracting_Number_Of_Employees_Followers
    ...
    \    BuiltIn.Run Keyword If    '${Current_Statuss}'=='InQueue'    Processing_Inqueue_Records    ${City_List}    ${State_List}    ${ID_List}
    \    ...    ${count1}    ${i}    ${Current_Status}
...
Inner_For_Loop_Extracting_Emploees_Via_State_Name
    [Arguments]    ${ID_Number}    ${i}    ${State_Name}    @{Url_New_List}
...
Status_Verification
    [Arguments]    ${ID_Number}    ${i}    ${State_Name}
    Select Window    MAIN
    ...
    ${href}=    BuiltIn.Run Keyword And Ignore Error    Inner_For_Loop_Extracting_Emploees_Via_State_Name    ${ID_Number}    ${i}    ${State_Name}    @{Url_New_List}
    ...    BuiltIn.Return From Keyword
    ${IsEmployeeContainsHref}=    Get From List    ${href}    0
    ${HREF_EMPLOYEE}=    Get From List    ${href}    1
    BuiltIn.Run Keyword If    '${IsEmployeeContainsHref}'=='PASS'    BuiltIn.Run Keyword And Ignore Error    Inner_For_Loop_Extracting_Followers_Via_State_Name    ${ID_Number}    ${HREF_EMPLOYEE}    ${i}
    ...    ${State_Name}

Processing_Inqueue_Records
    [Arguments]    ${City_List}    ${State_List}    ${ID_List}    ${count1}    ${i}    ${Current_Status}
    Select Window    MAIN
    ${City_Name}=    Get From List    ${City_List}    ${count1}
    ${State_Name}=    Get From List    ${State_List}    ${count1}
    ${ID_Number}=    Get From List    ${ID_List}    ${count1}
    DatabaseLibrary.Query    SELECT GET_LOCK(${ID_Number},10);
    DatabaseLibrary.Execute Sql String    update urs_aa.linkedin_robot set status ='Processing' where id=${ID_Number};
    DatabaseLibrary.Query    SELECT RELEASE_LOCK(${ID_Number);
    Input Text    css=input[name='q']    site:linkedin.com ${i} ${City_Name} employees
    Sleep    60s
    Click Element    css=button[aria-label='Google Search']
    Sleep    60s
    @{Url_List}    SeleniumLibrary.Get WebElements    css=div.r>a
    ${href}=    BuiltIn.Run Keyword And Ignore Error    Inner_For_Loop_Extracting_Employees_Via_City    ${ID_Number}    ${i}    @{Url_List}    BuiltIn.Return From Keyword
    ${IsEmployeeContainsHref}=    Get From List    ${href}    0
    ${HREF_EMPLOYEE}=    Get From List    ${href}    1
    BuiltIn.Run Keyword If    '${IsEmployeeContainsHref}'=='PASS'    BuiltIn.Run Keyword And Ignore Error    Inner_For_Loop_Extracting_Followers_Via_City    ${ID_Number}    ${HREF_EMPLOYEE}    ${i}
    ...    ${City_Name}
    ${Current_Statuss}=    Get From List    ${Current_Status}    ${count1}
    BuiltIn.Run Keyword If    '${Current_Statuss}'=='Processing'    Status_Verification    ${ID_Number}    ${i}    ${State_Name}
    ${Current_Statuss}=    Get From List    ${Current_Status}    ${count1}
