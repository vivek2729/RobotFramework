...
Extracting_Details_From_URL
    ...
    DatabaseLibrary.Execute Sql String    update nbcu_pilot.MSNBC_Morning_Joe set Title= '${Title}' where id=${Count3}
    ...
    DatabaseLibrary.Execute Sql String    update nbcu_pilot.MSNBC_Morning_Joe set Date= '${Date}' where id=${Count3}
    ${Description}=    Get Text    //p[contains(@class,'dekText')]
    ${Description}=    String.Replace String    ${Description}    '    ''
    DatabaseLibrary.Execute Sql String    update nbcu_pilot.MSNBC_Morning_Joe set Description= '${Description}' where id=${Count3}
    ${Category}=    Get Text    //h2[contains(@class,'articleTitleSection category')]/a/span
    ${Category}=    String.Replace String    ${Category}    '    ''
    DatabaseLibrary.Execute Sql String    update nbcu_pilot.MSNBC_Morning_Joe set Category= '${Category}' where id=${Count3}
...
Extracting_Details_From_Main_Page
    ...
    DatabaseLibrary.Execute Sql String    INSERT INTO nbcu_pilot.MSNBC_Morning_Joe (`Explored_Texts`) VALUES ('${Explore_Links}')
    Sleep    5s
    ${list}=    DatabaseLibrary.Query    select max(id) from nbcu_pilot.MSNBC_Morning_Joe;
    ...
    DatabaseLibrary.Execute Sql String    update nbcu_pilot.MSNBC_Morning_Joe set Video_Rendered_Before= '${Video_Rendered_Before}' where id=${Count3}
    Sleep    5s
    ...
    DatabaseLibrary.Execute Sql String    update nbcu_pilot.MSNBC_Morning_Joe set URL= '${URL}' where id=${Count3}
...
Duration_Value
    ...
    DatabaseLibrary.Execute Sql String    update nbcu_pilot.MSNBC_Morning_Joe set Duration= '${Duration}' where id=${Count3}
