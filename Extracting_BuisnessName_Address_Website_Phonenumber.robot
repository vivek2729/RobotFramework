...
Extracting_Details
    ...
    \    ${list}=    DatabaseLibrary.Query    select id,company_name, city, complete_state ,status,address,zip from urs_aa.google where id='20';
    ...
    \    ...    ${State_Name}
    \    ${Status_list}=    DatabaseLibrary.Query    select status from urs_aa.google where id=${ID_Number};
    \    ${Current_Status_List}    Evaluate    [x[0] for x in ${Status_list}]
    \    ${Updated_Status}=    Get From List    ${Current_Status_List}    ${count}
    \    BuiltIn.Run Keyword If    '${Current_Status}'=='${Updated_Status}'    Run Keyword And Ignore Error    Extracting__Details_With_Name_City_And_State    ${ID_Number}    ${Name}
    \    ...    ${City_Name}    ${State_Name}
    \    ${Status_list}=    DatabaseLibrary.Query    select status from urs_aa.google where id=${ID_Number};
    \    ${Current_Status_List}    Evaluate    [x[0] for x in ${Status_list}]
    \    ${Updated_Status}=    Get From List    ${Current_Status_List}    ${count}
    \    BuiltIn.Run Keyword If    '${Current_Status}'=='${Updated_Status}'    Run Keyword And Ignore Error    Extracting__Details_With_Name_And_ZipCode    ${ID_Number}    ${Name}
    \    ...    ${zip}
    \    ${Status_list}=    DatabaseLibrary.Query    select status from urs_aa.google where id=${ID_Number};
    \    ${Current_Status_List}    Evaluate    [x[0] for x in ${Status_list}]
    \    ${Updated_Status}=    Get From List    ${Current_Status_List}    ${count}
    \    BuiltIn.Run Keyword If    '${Current_Status}'=='${Updated_Status}'    Run Keyword And Ignore Error    Extracting__Details_With_Name_Address_City    ${ID_Number}    ${Name}
    \    ...    ${Address}    ${City_Name}
...
Extracting__Details_With_Name_Address_City_And_State
    ...
    Run Keyword If    '${bool}'=='PASS'    DatabaseLibrary.Execute Sql String    update urs_aa.google set google_company_name= '${New_Buisness_Name}' where id=${ID_Number};
    Run Keyword If    '${bool}'=='PASS'    DatabaseLibrary.Execute Sql String    update urs_aa.google set status ='Processed' where id=${ID_Number};
    ...
    Run Keyword If    '${bool}'=='PASS'    DatabaseLibrary.Execute Sql String    update urs_aa.google set google_phone= '${New_Phone_Number}' where id=${ID_Number};
    Run Keyword If    '${bool}'=='PASS'    DatabaseLibrary.Execute Sql String    update urs_aa.google set status ='Processed' where id=${ID_Number};
    ...
    Run Keyword If    '${bool}'=='PASS'    DatabaseLibrary.Execute Sql String    update urs_aa.google set google_website= '${New_Website_Link}' where id=${ID_Number};
    Run Keyword If    '${bool}'=='PASS'    DatabaseLibrary.Execute Sql String    update urs_aa.google set status ='Processed' where id=${ID_Number};
    ...
    Run Keyword If    '${bool}'=='PASS'    DatabaseLibrary.Execute Sql String    update urs_aa.google set google_address= '${New_Address}' where id=${ID_Number};
    Run Keyword If    '${bool}'=='PASS'    DatabaseLibrary.Execute Sql String    update urs_aa.google set status ='Processed' where id=${ID_Number};
